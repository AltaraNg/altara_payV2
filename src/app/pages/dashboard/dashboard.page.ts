...
export class DashboardPage implements OnInit {
  ...
  getBusinessTypes() {
    ...
    return this.http.get(this.env.NEW_API_URL + '/api/business_type', options
      ...
      this.businessTypes = res['data']['data'].filter((data) => data.name.includes('Altara Pay') && data.status).sort((a,b) => a.name > b.name ? 1 : -1);
    })
  }
  ...
  createOrder() {
    ...
    const data = {
      ...
      "down_payment": this.fifthFormGroup.value.makePayment == "2" ? +(+this.fifthFormGroup.value.enterAmount).toFixed(2) : +(+this.sixthFormGroup.value.downPayment).toFixed(2),
  ...
  getCalc() {
    try {
      ...
      const { total, actualDownpayment, actualRepayment } =
        this.eightFormGroup.value.business_type_id.name.includes('Cash Loan') || this.eightFormGroup.value.business_type_id.name.includes('Rentals') ? cashLoan(
